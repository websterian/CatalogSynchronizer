...
namespace Plugin.Sample.Payments.Braintree
{
    ...
    public class UpdateFederatedPaymentBlock : PipelineBlock<Order, Order, CommercePipelineExecutionContext>
    {
        ...
        {
            Condition.Requires(arg).IsNotNull($"{Name}: order can not be null.");

            var order = arg;
            if (!order.HasComponent<OnHoldOrderComponent>())
            {
                var invalidOrderStateMessage = $"{Name}: Expected order in '{context.GetPolicy<KnownOrderStatusPolicy>().OnHold}' status but order was in '{order.Status}' status";
                context.Abort(
            ...
            try
            {
                ...
                if (order.HasComponent<FederatedPaymentComponent>())
                {
                    ...
                    {
                        context.Logger.LogInformation($"{Name} - Void Payment succeeded: {orderPayment.Id}");
                        orderPayment.TransactionStatus = voidResult.Target.Status.ToString();
                        await GenerateSalesActivity(order, orderPayment, context).ConfigureAwait(false);
                    }
                    else
                    {
                        ...
                        context.Abort(
                            await context.CommerceContext.AddMessage(
                                ...
                                $"{Name}. Payment void failed for transaction {orderPayment.TransactionId}: {errorMessages}").ConfigureAwait(false),
                            context);
