...
namespace Plugin.Sample.AdventureWorks
{
    ...
    public class InitializeEnvironmentPromotionsBlock : PipelineBlock<string, string, CommercePipelineExecutionContext>
    {
        ...
        private async Task CreateCartFreeShippingPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateCartExclusive5PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNEC5P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateCartExclusive5OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNEC5A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        {
            var promotion = await _addPromotionPipeline.Run(new AddPromotionArgument(book, "Cart15PctOffCouponPromotion", DateTimeOffset.UtcNow.AddDays(-5), DateTimeOffset.UtcNow.AddYears(1), "15% Off Cart (Coupon)", "15% Off Cart (Coupon)")
            {
                DisplayName = "15% Off Cart (Coupon)",
                Description = "15% off Cart with subtotal of $50 or more (Coupon)"
            }, context).ConfigureAwait(false);

            promotion = await _addQualificationPipeline.Run(new PromotionConditionModelArgument(promotion, new ConditionModel
            {
                ConditionOperator = "And",
                Id = Guid.NewGuid().ToString(),
                LibraryId = CartsConstants.CartSubtotalCondition,
            ...
            promotion = await _addBenefitPipeline.Run(new PromotionActionModelArgument(promotion, new ActionModel
            {
                Id = Guid.NewGuid().ToString(),
                LibraryId = CartsConstants.CartSubtotalPercentOffAction,
                Name = CartsConstants.CartSubtotalPercentOffAction,
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNC15P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateSamplePrivateCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPrivateCouponPipeline.Run(new AddPrivateCouponArgument(promotion, "SPCP_", "_22", 15), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        ...
        private async Task CreateCart10PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context, DateTimeOffset date)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNC10P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateCart10OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context, DateTimeOffset date)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNC10A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLinePantsPricePromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "AWSELLPRICE"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLineSahara5OffPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPromotionItemPipeline.Run(new PromotionItemArgument(promotion, "Adventure Works Catalog|AW114 06|"), context).ConfigureAwait(false);
        ...
        private async Task CreateLineExclusive20PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNEL20P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLineExclusive20OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNEL20A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLine5PctOffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNL5P"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
        ...
        private async Task CreateLine5OffCouponPromotion(PromotionBook book, CommercePipelineExecutionContext context)
        {
            ...
            promotion = await _addPublicCouponPipeline.Run(new AddPublicCouponArgument(promotion, "RTRNL5A"), context).ConfigureAwait(false);
            promotion.SetComponent(new ApprovalComponent(context.GetPolicy<ApprovalStatusPolicy>().Approved));
            await _persistEntityPipeline.Run(new PersistEntityArgument(promotion), context).ConfigureAwait(false);
        }
    }
